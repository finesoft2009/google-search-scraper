...
class GoogleSearch {
    ...
    static makeRequest(term_1) {
        return __awaiter(this, arguments, void 0, function* (term, options = {}) {
            ...
            const params = new URLSearchParams(Object.assign(Object.assign({ q: term, num: numResults.toString(), hl: lang, start: start.toString(), safe: safe }, (region && { gl: region })), { ie: 'UTF-8', oe: 'UTF-8', gws_rd: 'cr', filter: '0', gbv: '1' // Показывать все результаты, включая дубликаты
             }));
            if (lang !== 'en') {
            ...
            })), { validateStatus: (status) => status === 200 || status === 302 || status === 429, maxRedirects: 5, decompress: true });
            try {
                // Добавляем небольшую задержку, чтобы имитировать поведение человека
                yield new Promise(resolve => setTimeout(resolve, Math.random() * 2000 + 1000)); // 1-3 секунды
